...
class _AddClientState extends State<AddClient> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Center(child: buildSignUpBody(height, width)),
    );
  }
  ...
  setValue(value, selected) {
    switch (value) {
      ...
      case "municipality":
        return munic = selected;
    }
  }

  Widget buildDropdownButtonFormField<T>(
      double width, double height, List<T> table, String hint,
      {String defaultValue, String field, bool callFunction}) {
    return Container(
        width: width * 0.9,
        height: height * 0.09,
        ...
        child: DropdownButton<String>(
            ...
            hint: Padding(
              padding: const EdgeInsets.all(8.0),
              child: Text(getValue(field) ?? hint,
                ...
                child: Center(
                  child: Container(
                      ...
                      child: Padding(
                        ...
                        child: Text(
                          f.toString(),
                          style: TextStyle(color: myColor, fontSize: 20.0),
                        ),
                      )),
                ),
              );
            ...
            onChanged: (selected) async {
              setState(() {
                if (callFunction) {
                  munics =
                      govs.firstWhere((f) => f.toString() == selected).munics;
                }
        ...
        child: Column(
          children: <Widget>[
            ...
            formField(height, width,
                ...
                tag: "last_name"),
            formField(height, width,
                icon: fv.MaterialCommunityIcons.id_card,
                label: "CIN",
                tag: "cin"),
            formField(height, width,
            ...
            formField(height, width,
                ...
                label: "Télephone",
                tag: "phone"),
            buildDropdownButtonFormField<Gender>(
                width, height, [Gender("Homme"), Gender("Femme")], "genre",
                field: "gender", callFunction: false, defaultValue: "Male"),
            formField(height, width,
                icon: fv.MaterialCommunityIcons.phone,
                label: "zip_code",
                tag: "zip_code"),
            buildDropdownButtonFormField<Gov>(width, height, govs, "governorat",
                field: "governorate",
                callFunction: true,
                defaultValue: "Not assigned"),
            mySizedBox20(),
            buildDropdownButtonFormField<Municipality>(
                width, height, munics, "municipalité",
                field: "municipality",
                callFunction: false,
                defaultValue: "Not assigned"),
            mySizedBox20(),
            formField(height, width,
                icon: Icons.my_location, label: "addresse", tag: "address"),
            ChangeNotifierProvider.value(
              value: model,
              child: Consumer<MotherModel>(
                  builder: (_, model, ___) => model.bload
                      ? MyButton(
                          fill: true,
                          function: add,
                          label: "Add",
  ...
  add() async {
    if (_fomrKey.currentState.validate()) {
      ...
      dynamic result = await model.addClient(info,
          cid: widget.clientId, pid: widget.parentId, aid: widget.affiliateId);
      print(result);
      if (result == null) {
        showErrorDialog(
            mssj: "un problème est survenu lors de l'ajout de la demande");
        return;
      } else {
  ...
  dynamic _validator(String value, {String label}) {
    switch (label) {
      case "Password":
        return validators.isAlphanumeric(value) && value.length >= 8
            ? null
            : "le mot de passe est non valide ";
      case "remax@gmail.com":
        return validators.isEmail(value) ? null : "l'email est non valide ";
      case "phone":
        return value.length == 8 ? null : "le numéro est non valide ";
      case "cin":
        return value.length == 8 ? null : "le cin est non valide ";
      case "zip_code":
        return value.length == 4 ? null : " 4 chiffres";
      case "city":
        return value.length >= 4 ? null : "4 char minimun";
      default:
